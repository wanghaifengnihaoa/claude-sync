...
export function expandTilde(p, home = os.homedir()) {
  if (typeof p === 'string' && p.startsWith('~')) {
    // Expand a LEADING ~ to the home directory using the platform's native path
    // joining, so a Windows user typing ~/x gets C:\Users\x and a Mac user gets
    // /Users/x. Only a leading ~ is a home reference; an interior ~ (e.g.
    // iCloud's com~apple~) must never trigger this.
    const rest = p.slice(1);
    if (rest.startsWith('/') || rest.startsWith('\\')) {
      // Strip the leading separator: on Windows path.join('C:\\Users\\x', '/foo')
      // would resolve to 'C:\\foo' because '/' is treated as root-relative.
