  ...
  await tar.create(
    {
      ...
      cwd: sourceDir,
      filter: (filePath) => {
        // Check every path segment against the exclude set. Split on both '/'
        // and '\\': tar reports entry paths in POSIX form, which on Windows
        // never matches path.sep (a backslash), so the segments never align
        // with the exclude set and excluded dirs sneak into the bundle.
