...
// execSync used only for 'git rev-parse' and 'git remote get-url' with
...
function getGitInfo(dirPath) {
  try {
    const remote = execSync('git remote get-url origin', { cwd: dirPath, encoding: 'utf-8', timeout: 5000 }).trim();
    const commit = execSync('git rev-parse HEAD', { cwd: dirPath, encoding: 'utf-8', timeout: 5000 }).trim();
    const branch = execSync('git rev-parse --abbrev-ref HEAD', { cwd: dirPath, encoding: 'utf-8', timeout: 5000 }).trim();
