 ...
 * @param {boolean} [clearOnDone] - when true, clear this render (restore cursor +
 *   erase below) before resolving, so a retry loop that calls pickFromList again
 *   starts clean instead of stacking a new copy on top of the old one. Leave off
 *   for one-shot prompts where the picked list should stay on screen.
...
export async function pickFromList(question, items, defaultItem, header, footer, clearOnDone = false) {
  ...
  return new Promise(resolve => {
    ...
    function finish(result) {
      if (clearOnDone) {
        // Clear this instance's render and return the cursor to its start, so a
        // following pickFromList (e.g. a Retry in a loop) draws fresh instead of
        // stacking on top of the previous render. Relies on SAVE_CURSOR/RESTORE_CURSOR
        // (already used for navigation), not on DEC \x1b7/\x1b8 which are unreliable
        // under Windows ConPTY.
