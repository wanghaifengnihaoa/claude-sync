...
export async function handlePlugins(plugins, claudeDir, strategy, { exec = execCli, scriptDir = path.join(os.homedir(), '.claude-sync-bundle') } = {}) {
  ...
  if (currentPluginsData.plugins && typeof currentPluginsData.plugins === 'object') {
    ...
    // toInstall/toUpdate are disjoint by construction (a plugin is either
    // missing or present-with-different-version), so one loop covers both.
    const keyFor = (name) =>
      Object.keys(currentPluginsData.plugins).find(k => pluginNameFromKey(k) === name) || name;
    for (const { name, version } of [...toInstall, ...toUpdate]) {
