...
import { prompt, promptYesNo } from './prompt.js';
...
export function resolveSymlinksInDir(dir) {
  ...
  // Security boundary: only dereference symlinks pointing within the staging dir or home dir
  const HOME = os.homedir();
  const ALLOWED_ROOTS = [path.resolve(dir), HOME];
  let entries;
  ...
  if (toUpdate.length > 0) {
    ...
    for (const { name, version } of toUpdate) {
      try {
        ...
        log('info', `  ✗ ${name} update failed: ${e.message}.`);
        failedOps.push(`claude plugin uninstall ${name} && claude plugin install ${name}@${version}`);
      }
    }
  }
