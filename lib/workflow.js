...
// can't launch directly (ENOENT — the classic Windows npm-bin trap). Route them
// through the shell there so the shims resolve via PATHEXT; elsewhere keep the
// direct-exec path (no shell, faster, no shell-quoting surprises).
function execCli(cmd, args, opts = {}) {
  return execFileSync(cmd, args, { ...opts, shell: process.platform === 'win32' });
}
...
export function getClaudeVersion() {
  try {
    const out = execCli('claude', ['--version'], { encoding: 'utf-8', timeout: 300000 }).trim();
...
async function ensureClaudeVersion(sourceVersion) {
  ...
  if (!localVersion) {
    ...
    // Loop so a failed install never silently continues the pull — the user
    // must retry, skip (continue without Claude), or cancel.
    while (true) {
      ...
      if (choice.startsWith('Install')) {
        const ok = await installClaude(sourceVersion);
        if (ok) return { action: 'continue' };
        log('error', 'Install did not complete — choose again or cancel.');
        continue;
      }
      if (choice === 'Cancel pull') return { action: 'cancel' };
      // "Continue without Claude" — proceed without the CLI installed.
      return { action: 'continue' };
    }
  }
  ...
  // Upgrade/downgrade loop: a failed install must NOT silently continue the
  // pull — go around so the user can retry, accept the risk, or cancel.
  while (true) {
    const choice = await pickFromList(
      'What would you like to do?',
      [
    ...
    if (choice.startsWith(verb)) {
      const ok = await installClaude(sourceVersion);
      if (ok) {
        log('info', `Claude upgraded to v${sourceVersion}; proceeding with pull.`);
        return { action: 'continue' };
      }
      log('error', `${verb} to v${sourceVersion} did not complete — choose again or cancel.`);
      continue;
    }
    if (choice === 'Cancel pull') return { action: 'cancel' };
...
/**
 * Windows only: is any claude.exe process running? Replacing a running .exe
 * is impossible on Windows, so upgrading Claude Code while an instance is open
 * (including the very session that invoked claude-sync) is guaranteed to fail
 * with an EBUSY file-lock after npm retries for a long time. Detect it up
 * front so the user gets an instant, actionable message instead of a 120s
 * timeout. tasklist exits 0 only when a match is found; the no-match case
 * throws from execFileSync and is caught as "not running".
 */
function isClaudeExeRunning() {
  if (process.platform !== 'win32') return false;
  try {
    // tasklist.exe is a native exe — direct exec (no shell), so the /FI filter
    // (which contains spaces) stays a single argument; cmd.exe would strip the
    // quotes and tasklist would reject 'eq' as an invalid option.
    const out = execFileSync('tasklist', ['/FI', 'IMAGENAME eq claude.exe'], { encoding: 'utf-8' });
...
/**
 * Install a specific Claude Code version globally.
 * Returns true only if the install command succeeded AND `claude --version`
 * now reports the requested version. Never throws; callers must treat a
 * failed install as a failure (not silently continue the pull).
 */
export async function installClaude(version) {
  const pkg = `@anthropic-ai/claude-code@${version}`;
  console.log();
  console.log(`Installing ${pkg}...`);
  try {
    if (isClaudeExeRunning()) {
      console.log('⚠  claude.exe is currently running on this machine.');
      console.log('   Windows cannot replace a running .exe, so the upgrade would fail.');
      console.log('   Quit all Claude Code windows/sessions, then run manually:');
      console.log(`     npm install -g ${pkg}`);
      return false;
    }
    execCli('npm', ['install', '-g', pkg], { stdio: 'inherit', timeout: 300000 });
  } catch (e) {
    log('error', `npm install failed: ${e.message}`);
    const locked = process.platform === 'win32' && /EBUSY|locked|ETIMEDOUT/i.test(e.message);
    console.log(locked
      ? '⚠  Likely cause: claude.exe is in use (a Claude Code instance is running).'
  ...
  // version on PATH is unchanged (cache, perms, or a different install location).
  const now = getClaudeVersion();
  if (now !== version) {
    log('error', `Claude version is still ${now} after install (expected v${version}).`);
    console.log(`✗  Installed, but claude --version still reports ${now || 'no version'}.`);
    console.log('   Quit all Claude Code instances, then run manually:');
    console.log(`     npm install -g ${pkg}`);
    console.log();
    return false;
  }
...
export function applyPathReplacement(dir, sourceHome, targetHome) {
  ...
  // Apply replaceUserPath to every string leaf of a JSON value. JSON files go
  // through parse/walk/stringify instead of raw string replacement because a
  // Windows target home ("C:\Users\whf") contains backslashes that must be
  // escaped as \\ in JSON — a raw replace wrote "C:\Users\whf" verbatim,
  // which is invalid JSON (\U is not an escape) and broke every marketplace
  // registry (known_marketplaces.json) on Mac→Windows pulls.
  function replaceInStrings(value) {
    ...
    if (value && typeof value === 'object') {
      const out = {};
      for (const [k, v] of Object.entries(value)) out[k] = replaceInStrings(v);
      return out;
    }
    return value;
  }
      ...
      if (entry.isDirectory()) {
        ...
        try {
          if (entry.name.endsWith('.json')) {
            // Skip files that don't parse as JSON — raw-replacing those is
            // what corrupted known_marketplaces.json.
            const obj = JSON.parse(fs.readFileSync(fullPath, 'utf-8'));
            const newObj = replaceInStrings(obj);
            ...
            // Markdown keeps the raw replace.
          ...
          // skip binary/unreadable / unparseable JSON
        }
      }
    }
  }
    ...
    if (!fs.existsSync(dest) || strategy === 'cover') {
      if (skill.remote) {
        try {
          ...
          execCli('git', ['clone', skill.remote, dest], { stdio: 'pipe', timeout: 300000 });
          if (skill.commit) {
            try { execCli('git', ['checkout', skill.commit], { cwd: dest, stdio: 'pipe', timeout: 300000 }); } catch {}
          }
          if (skill.hasPackageJson && fs.existsSync(path.join(dest, 'package.json'))) {
            try { execCli('npm', ['install'], { cwd: dest, stdio: 'pipe', timeout: 300000 }); } catch {}
          }
  ...
  for (const skill of (skillsManifest.skills_sh || [])) {
    if (skill.source) {
      try {
        ...
        execCli('npx', ['skills', 'add', skill.source], { stdio: 'pipe', timeout: 300000 });
  ...
  if (toInstall.length > 0) {
    ...
    for (const { name, version } of toInstall) {
      try {
        ...
        execCli('claude', ['plugin', 'install', `${name}@${version}`], { stdio: 'pipe', timeout: 300000 });
  ...
  if (toUpdate.length > 0) {
    ...
    for (const { name, version, existing } of toUpdate) {
      try {
        ...
        try { execCli('claude', ['plugin', 'uninstall', name], { stdio: 'pipe', timeout: 300000 }); } catch {}
        execCli('claude', ['plugin', 'install', `${name}@${version}`], { stdio: 'pipe', timeout: 300000 });
        ...
        try {
          execCli('claude', ['plugin', 'install', `${name}@${existing}`], { stdio: 'pipe', timeout: 300000 });
