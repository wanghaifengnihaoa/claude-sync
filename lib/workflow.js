  ...
  if (toInstall.length > 0) {
    ...
    for (const { name, version } of toInstall) {
      try {
        ...
        execCli('claude', ['plugin', 'install', `${name}@${version}`], { stdio: 'pipe', timeout: 60000 });
  ...
  if (toUpdate.length > 0) {
    ...
    for (const { name, version, existing } of toUpdate) {
      try {
        ...
        try { execCli('claude', ['plugin', 'uninstall', name], { stdio: 'pipe', timeout: 30000 }); } catch {}
        execCli('claude', ['plugin', 'install', `${name}@${version}`], { stdio: 'pipe', timeout: 60000 });
        ...
        try {
          execCli('claude', ['plugin', 'install', `${name}@${existing}`], { stdio: 'pipe', timeout: 60000 });
