...
export function execCli(cmd, args, opts = {}, { platform = process.platform } = {}) {
  ...
  if (status !== 0) {
    ...
    // Match execFileSync's failure error: numeric status, string code, and
    // stdout/stderr carried for diagnostics.
    err.status = status;
...
import { createManifest, buildBundle, extractBundle, readManifest, writeManifest, hashFile, readPluginVersions, pluginNameFromKey } from './sync.js';
...
export async function handlePlugins(plugins, claudeDir, strategy, { exec = execCli, scriptDir = path.join(os.homedir(), '.claude-sync-bundle') } = {}) {
  const pluginsDir = path.join(claudeDir, 'plugins');
  ...
  const failedOps = [];
  // Successful installs/updates only — failed ones must NOT be recorded in
  ...
  if (toInstall.length > 0) {
    ...
    for (const { name, version } of toInstall) {
      try {
        ...
        exec('claude', ['plugin', 'install', `${name}@${version}`], { stdio: 'pipe', timeout: 60000 });
  ...
  if (toUpdate.length > 0) {
    ...
    for (const { name, version, existing } of toUpdate) {
      try {
        ...
        try { exec('claude', ['plugin', 'uninstall', name], { stdio: 'pipe', timeout: 30000 }); } catch {}
        exec('claude', ['plugin', 'install', `${name}@${version}`], { stdio: 'pipe', timeout: 60000 });
        ...
        try {
          exec('claude', ['plugin', 'install', `${name}@${existing}`], { stdio: 'pipe', timeout: 60000 });
  ...
  // the install/update loops: `claude plugin install` rewrites this file itself
  ...
  // entries this run installed/updated.
  ...
  if (currentPluginsData && currentPluginsData.plugins && typeof currentPluginsData.plugins === 'object') {
    ...
    // Manifest keys are bare names but the file keys are "name@marketplace" —
    // match an existing entry by bare name so an update rewrites the same key
    // instead of creating a second bare-name entry (CC format would then have
    // both shapes for one plugin). New installs fall back to the bare name.
    const keyFor = (name) =>
      Object.keys(currentPluginsData.plugins).find(k => pluginNameFromKey(k) === name) || name;
    for (const { name, version } of applied) {
      currentPluginsData.plugins[keyFor(name)] = [{ version, installedAt: new Date().toISOString() }];
    }
    ...
    let merged = {};
    try { merged = JSON.parse(fs.readFileSync(localPluginsPath, 'utf-8')) || {}; } catch {}
    if (typeof merged !== 'object' || merged === null || Array.isArray(merged)) merged = {};
    for (const { name, version } of applied) merged[name] = version;
