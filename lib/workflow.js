...
//
// `platform` is an injectable override so tests can force the win32 branch on any OS.
...
export async function handlePlugins(plugins, claudeDir, strategy, { exec = execCli, scriptDir = path.join(os.homedir(), '.claude-sync-bundle') } = {}) {
  ...
  if (rawPluginsData.plugins && typeof rawPluginsData.plugins === 'object') {
    ...
    const keyFor = (name) =>
      Object.keys(rawPluginsData.plugins).find(k => k.split('@')[0] === name) || name;
    for (const { name, version } of toInstall) {
      const key = keyFor(name);
      if (!rawPluginsData.plugins[key]) rawPluginsData.plugins[key] = [];
      rawPluginsData.plugins[key] = [{ version, installedAt: new Date().toISOString() }];
    }
